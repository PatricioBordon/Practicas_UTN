...
int main() {
	...
	float menor;
	float navion;
	int menorAvion, asivendtotal, asinovendtotal;
	float numeri;
	float porcentaje;
	float puntu;
	float puntual;
	...
	while ((navion!=0)) {
		...
		while (navion==anterior) {
			...
			cin >> puntualidad;
			asivendtotal+=asivend;
			asinovendtotal+=asinovend;
			if (puntualidad>0) {
			...
			capacidad =asivendtotal+asinovendtotal;
			cout << "Numero de avion" << endl;
			cin >> navion;
		}
		porcentaje = asivendtotal*100/capacidad;
		if (menor==0) {
			menor = porcentaje;
			menorAvion = anterior;
		} else {
	...
	cout << "Avion con menor porcentaje de asientos ocupados: " << menorAvion << endl;
